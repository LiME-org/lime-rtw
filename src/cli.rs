...
#[cfg(target_os = "linux")]
fn parse_target_tgid(args: &[String]) -> Option<i32> {
    match args {
        [pid] => pid.parse::<i32>().ok().filter(|pid| *pid > 0),
        _ => None,
    }
}
...
#[cfg(target_os = "linux")]
fn parse_trace_target(args: &[String]) -> Option<TraceTarget<'_>> {
    if args.is_empty() {
        None
    } else if let Some(target_tgid) = parse_target_tgid(args) {
        Some(TraceTarget::TargetTgid(target_tgid))
    } else {
        Some(TraceTarget::Command(args))
    }
}
...
pub enum LimeSubCommand {
    ...
    Trace {
        ...
        /// Override PID namespace detection by providing the inode of the target
        /// PID namespace. This is needed when LiME runs on the host but the traced
        /// process is inside a container.
        #[clap(long = "target-pid-ns", value_name = "INODE")]
        target_pid_ns: Option<u32>,

        /// Command to trace, or a single target TGID/TID to trace an existing process or thread.
        /// By default, only the events generated by the invoked command and its child will be traced.
        /// Note that scheduling policy still apply.
        /// If none is given lime-rtw will trace until
        /// it receives SIGINT.
        #[clap(value_name = "CMD_OR_TGID")]
        trace_target_args: Vec<String>,
...
impl CLI {
    #[cfg(target_os = "linux")]
    pub fn trace_target(&self) -> Option<TraceTarget<'_>> {
        match &self.command {
            ...
            | LimeSubCommand::Extract {
                trace_target_args, ..
            } => parse_trace_target(trace_target_args),
            _ => None,
        }
    }
    ...
    #[cfg(target_os = "linux")]
    pub fn target_pid_ns(&self) -> Option<u32> {
        match &self.command {
            LimeSubCommand::Trace { target_pid_ns, .. }
            | LimeSubCommand::Extract { target_pid_ns, .. } => *target_pid_ns,
            _ => None,
        }
    }
...
mod tests {
    #[cfg(target_os = "linux")]
    use crate::cli::{LimeSubCommand, TraceTarget, CLI};
    #[cfg(target_os = "linux")]
    use clap::Parser;
    ...
    fn test_cli_trace() {
        ...
        match cli.command {
            LimeSubCommand::Trace {
                ...
                assert!(trace_target_args.len() == 3);
                assert!(!trace_all)
            }
            _ => unreachable!(),
        }
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn test_cli_trace_target_parsing() {
        let cli = CLI::parse_from(["lime-rtw", "trace", "--", "1234"]);

        match cli.trace_target() {
            Some(TraceTarget::TargetTgid(target_tgid)) => assert_eq!(target_tgid, 1234),
            _ => unreachable!(),
        }

        let cli = CLI::parse_from(["lime-rtw", "trace", "--", "a", "b", "c"]);

        match cli.trace_target() {
            Some(TraceTarget::Command(cmd)) => {
                assert_eq!(cmd, ["a".to_string(), "b".to_string(), "c".to_string()]);
            }
            _ => unreachable!(),
        }

        let cli = CLI::parse_from(["lime-rtw", "extract", "--", "1234"]);
